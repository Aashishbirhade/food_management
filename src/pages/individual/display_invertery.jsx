...
function DisplayItem() {
  const { inventory, setInventory } = useUserContext();  // Destructure inventory and setInventory
  const [filter, setFilter] = useState('');
  const [search, setSearch] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  ...
  const handleFilterChange = (e) => {
    ...
    if (selectedFilter === 'near-expiry') {
      ...
      setFilteredData(inventory.filter((item) => parseInt(item.quantity) > 300));
    } else {
  ...
  const handleAddInventory = () => {
    const newItem = {
      ...
      name: 'New Item', // You can customize this to accept dynamic inputs
      quantity: '100gm',
      manufactureDate: '01/01/24',
      expiryDate: '01/01/25',
      image: './src/images/paneer.webp',
    };
